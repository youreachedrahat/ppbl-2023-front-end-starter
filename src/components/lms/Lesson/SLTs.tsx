import { Box, Container, Divider, Heading, List, ListItem, Text, Flex, useColorModeValue } from "@chakra-ui/react";
...
const SLTsItems = ({ moduleTitle, moduleNumber }: SLTsItemsProps) => {
  ...
  return (
    <Box>
      ...
      <Heading size="2xl" color={textColorBlue}>{currentModule.title}</Heading>
        ...
        <List lineHeight="2">
            {items.map((item, index) => {
                ...
                <ListItem key={id}>
                    ...
                    <Text color={textColorBlue} style={{ marginRight: 4 }}>{id}</Text>
                    <Text> : {SLT}</Text>
                    </Flex>
                </ListItem>
                );
