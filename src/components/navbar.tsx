...
export default function WithSubnavigation() {
  ...
  return (
    <Box>
      <Flex
        bg={useColorModeValue("theme.white", "theme.darkGray")}
        color={useColorModeValue("theme.darkGray", "theme.white")}
        minH={"60px"}
        py={{ base: 2 }}
        px={{ base: 4 }}
        borderBottom={1}
        borderStyle={"solid"}
        borderColor={useColorModeValue("theme.darkGray", "theme.darkGray")}
        align={"center"}
      >
        ...
        <Flex flex={{ base: 1 }} justify={{ base: "center", md: "start" }}>
          <Link href="/">
            <Image
              src="https://gimbalabs-docs.vercel.app/img/g-black.svg"
              alt="gimbalabs logo"
              width={25}
              height={25}
            />
          ...
          <Text
            ...
            fontFamily={"heading"}
            color={useColorModeValue("theme.darkGray", "white")}
          >
            <Link href="/">Gimbalabs</Link>
          </Text>
...
const DesktopNav = () => {
  const linkColor = useColorModeValue("theme.darkGray", "theme.darkGray");
  const linkHoverColor = useColorModeValue("theme.darkGray", "white");
  const popoverContentBgColor = useColorModeValue("white", "theme.darkGray");
...
const DesktopSubNav = ({ label, href, subLabel }: NavItem) => {
  return (
    <Link
      ...
      p={2}
      rounded={"md"}
      _hover={{ bg: useColorModeValue("pink.50", "theme.darkGray") }}
    >
...
const MobileNav = () => {
  return (
    <Stack bg={useColorModeValue("white", "theme.darkGray")} p={4} display={{ md: "none" }}>
      {NAV_ITEMS.map((navItem) => (
...
const MobileNavItem = ({ label, children, href, childrenHasChildren }: NavItem) => {
  ...
  return (
    <Stack spacing={4}>
      ...
      >
        <Text fontWeight={600} color={useColorModeValue("theme.darkGray", "theme.darkGray")}>
          {label}
        </Text>
        {hasChildren && (
          <Icon
...
const MobileNavSubItem = ({ label, href }: NavItem) => {
  return (
    <Link
      ...
      _hover={{
        textDecoration: "none",
        bg: useColorModeValue("theme.darkGray", "theme.darkGray"),
      }}
    >
      {label}
    </Link>
