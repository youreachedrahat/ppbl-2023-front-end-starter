...
This token pair represents a partial implementation of the design pattern suggested in [CIP 68 - Datum Metadata Standard](https://cips.cardano.org/cips/cip68/). Throughout PPBL 2023, we will work together to build our understanding of CIP 68, and we will decide whether it sufficiently supports the design of the PPBL 2023 Token. We will decide whether to fully adopt the CIP, or suggest changes to it, or draft a CIP of our own.

<br />
---

## What is a "CIP"?
[CIP stands for "Cardano Improvement Proposal"](https://cips.cardano.org/), and the CIP process is an important feature of Cardano.

We will continue to investigate different CIPs in this course. When we do, we will encounter new questions about protocol governance, standards, and more.
...
We are able to update the metadatum associated with your PPBL 2023 Token. In this Mastery Assignment, you will see how.
...
At [Live Coding on 2023-0000](/live-coding), we will discuss some of the ways that CIP-68 is *not* currently applied to the PPBL 2023 Token.
...
In upcoming modules we will look at all of the above in detail.

For now, here is what we want you to notice:
1. `"int": 5` in the on-chain datum correspeonds to `luckyNumber` in the Plutus type.
2. `"list": []` in the on-chain datum correspeonds to `completedModules` in the Plutus type.
...
- As this course progresses, we will add to your list of completed modules.
...
A Plutus validator stipulates that you will be able to change the Lucky Number for ONLY YOUR token. You review the [source code of the validator here](https://gitlab.com/gimbalabs/ppbl-2023/ppbl-plutus-starter-2023/-/blob/main/src/ContribToken/Validator.hs). For now, you do not need to understand everything written in this PlutusTx validation code.

In this assignment, we will use the compiled validation script to build a transaction.
...
Which `CONTRIBUTOR_REFERENCE_ASSET` is yours? There are few ways to find out, and we will leave that to you. Note that there are web-based tools [like this that convert from hexadecmial to text strings](https://string-functions.com/hex-string.aspx)
...
If you complete steps 1 through 4 correctly, you will be able to build this transaction. If you get errors, you might have to double check some of the variables. As always, please bring your questions to Live Coding or to Gimbalabs Discord.
...
## Reference Scripts
You just build a transaction using an on-chain reference to a Plutus Script. This is another big idea that we will discuss in the weeks to come!
